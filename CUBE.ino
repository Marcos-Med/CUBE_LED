...
#define TEXT 6
#define LIT 7
#define SANTA_PROFILE 8
#define CHRISTMAS_TREE 9
#define MERRY_CHRISTMAS 10
...
#define TIME_SANTA_PROFILE 300
#define TIME_CHRISTMAS_TREE 200
...
void loop() {
  ...
  switch (currentEffect) {
    ...
    case LIT: lit(); break;
    case SANTA_PROFILE: santaProfileEffect(); break;
    case CHRISTMAS_TREE: christmasTreeEffect(); break;
    case MERRY_CHRISTMAS: merryChristmasEffect(); break;
    default: rain();
  }
...
void renderCube() { // rendiza o estado calculado pelas animações
  for (uint8_t i = 0; i < 8; i++) {
    digitalWrite(SS, LOW); // sinaliza ao SPI o envio de dados
    SPI.transfer(0x01 << i);
    for (uint8_t j = 0; j < 8; j++) {
      SPI.transfer(cube[i][j]);
    }
    digitalWrite(SS, HIGH); //finaliza envio
  }
}
...
void santaProfileEffect(){
  if(loading){
    clearCube();
    planePosition = random(0, 2) * 7;
    for(uint8_t i = 0; i < 8; i++){
      cube[i][planePosition] = santaProfile[i];
    }
    if(planePosition == 0){
      planeDirection = NEG_Z;
    }
  ...
  timer++;
  if(timer > TIME_SANTA_PROFILE){
    timer = 0;
    shift(planeDirection);
    if(planeDirection == POS_Z){
      planePosition--;
      if(planePosition == 0){
        planeDirection = NEG_Z;
      }
...
void merryChristmasEffect(){
  uint8_t message[] = {
    F, E, L, I, Z, SPACE, N, A, T, A, L
  };
  if(loading){
    clearCube();
    charPosition = -1;
  ...
  if(timer > TEXT_TIME){
    ...
    if (charPosition == 0) {
      for (uint8_t i = 0; i < 8; i++) {
        cube[i][0] = font[message[charCounter]][i];
      }
    }
  }
}
...
void drawTree(){
  for(uint8_t y = 1; y < 4; y++){
    for(int8_t x = y; x <= 7 - y; x++){
      for(int8_t z = y; z <= 7 - y; z++){
        setVoxel(x, y, z);
      }
    }
  }
